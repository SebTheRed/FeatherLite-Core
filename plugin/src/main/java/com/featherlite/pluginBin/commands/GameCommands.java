...
public class GameCommands implements TabCompleter {
    ...
    private void handleCreateCommand(Player player, String[] args) {
        ...
        if (args.length < 4) {
            player.sendMessage("Usage: /game create \"<game-name>\" <public|false> <world>");
            return;
        }
        ...
        try {
            isInstancePublic = Boolean.parseBoolean(args[2]);
        } catch (Exception e) {
            player.sendMessage("Invalid value for public/private flag. Use true or false.");
            return;
        }
    ...
    private String extractQuotedArgument(String[] args, int startIndex) {
        ...
        for (int i = startIndex; i < args.length; i++) {
            ...
            if (arg.startsWith("\"")) {
                ...
                return null; // Not a quoted argument
            }
        }

        return inQuotes ? null : quoted.toString().trim(); // Return null if quotes are unbalanced
    }
